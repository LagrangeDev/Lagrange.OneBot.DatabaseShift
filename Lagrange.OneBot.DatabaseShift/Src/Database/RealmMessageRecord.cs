...
using Lagrange.OneBot.DatabaseShift.Utilities;
using MessagePack;
...
public partial class RealmMessageRecord : IRealmObject {
    ...
    public static implicit operator RealmMessageRecord(LiteDBMessageRecord record) => new() {
        Id = record.MessageHash,
        Sequence = record.Sequence,
        ClientSequence = record.ClientSequence,
        MessageId = record.MessageId,
        Time = record.Time,
        FromUin = record.FriendUin,
        ToUin = record.GroupUin == 0 ? record.GroupUin : record.TargetUin,
        Entities = MessagePackSerializer.Serialize(record.Entities, OPTIONS)
    };

    public static implicit operator RealmMessageRecord(MessageChain chain) => new() {
    ...
    public static implicit operator MessageChain(RealmMessageRecord record) {
        var chain = record.Type switch {
            MessageType.Group => MessageChainUtility.MessageChainGroupCtor(
                (uint)record.ToUin,
                (uint)record.FromUin,
            ...
            MessageType.Temp or
            MessageType.Friend => MessageChainUtility.MessageChainFriendCtor(
                (uint)record.FromUin,
                string.Empty,
                string.Empty,
                (uint)record.ToUin,
        ...
        MessageChainUtility.MessageChainTimeSetter(chain, record.Time.DateTime);
